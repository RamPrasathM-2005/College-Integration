...
const CourseCard = ({ course, onEdit, onDelete }) => (
  <div className="p-4 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-300 transition-all cursor-pointer group">
    ...
    {/* Buttons in footer for better position */}
    <div className="flex justify-end gap-2 pt-2 border-t border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity">
      <button onClick={(e) => { e.stopPropagation(); onEdit(course); }} className="p-2 hover:bg-yellow-100 rounded text-sm flex items-center gap-1">
        <Edit className="w-4 h-4 text-yellow-600" />
        Edit
      </button>
      <button onClick={(e) => { e.stopPropagation(); onDelete(course.courseId); }} className="p-2 hover:bg-red-100 rounded text-sm flex items-center gap-1">
